...
import { executeCommand } from '../src/engine/executor.js';
import { validateStep } from '../src/engine/validator.js';
...
test('validator: files-staged validates multiple files', async () => {
    ...
    try {
        ...
        const result = await validateStep({ type: 'files-staged', files: ['file1.txt', 'file2.txt'] }, testDir);
        assert.strictEqual(result.passed, true);
    }
    finally {
        await cleanup(testDir);
